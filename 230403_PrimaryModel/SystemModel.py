...
def BioGasModel(DesignVector,*parameters):
    ...
    # manure weight times the distance THAT manure is shipped
    movement_kg_km = (MovementsKg.to_numpy() * MovementsKm.to_numpy()).sum().sum()
        
    total_distance_km                   = MovementsKm.sum().sum()
    total_manure_processed_kg_per_day = MovementsKg.sum().sum()
    ...
    transport_out = (movement_kg_km, total_manure_transported_kg_per_day, total_manure_processed_kg_per_day) #(total_distance_km, total_manure_transported_kg_per_day, total_manure_processed_kg_per_day)
    ...
    annual_net_income, Costs_tup, rev_dict, annualize_inv_cost, annual_net_income, annual_revenue = im.net_income(ImpactParameters,
              total_pipe_km, 
              total_upflow_cap_m3,
              transport_out,
              bio_out2, 
    ...
    # appending MethProd, BiogasProd, annualize_inv_cost, annual_net_income, annual_revenue to the end of this tuple
    return annual_net_income, Costs_tup, rev_dict, MovementsKg, net_co2_offset_kg_per_year, MethProd, BiogasProd, annualize_inv_cost, annual_net_income, annual_revenue
...
def CreateDemand(Supply,N,fraction_prod,random = False):
    ...
    Demand   = pd.DataFrame(Supply.values,Supply.index,['Demand0'])
    """
